...
namespace uapi-sdk-csharp.Api
{
    ...
    public interface INetworkApi : IApi
    {
        ...
        /// <param name="source">查询结果类型。不传时返回标准结果；如果设置为 &#x60;commercial&#x60;，将返回更完整的地理位置信息，但响应时间可能会稍长。 (optional)</param>
        /// <param name="cancellationToken">Cancellation Token to cancel the request.</param>
    ...
    public sealed partial class NetworkApi : INetworkApi
    {
        ...
        /// <summary>
        /// 查询 IP 想知道一个IP地址或域名来自哪里？这个接口可以帮你定位它。默认返回标准结果；如果传 &#x60;source&#x3D;commercial&#x60;，可以返回更完整的位置信息。  ## 功能概述 提供一个公网IPv4、IPv6地址或域名，我们会查询并返回它的地理位置（国家、省份、城市）、经纬度、以及所属的运营商（ISP）和自治系统（ASN）信息。这在网络安全分析、访问来源统计等领域非常有用。  当传 &#x60;source&#x3D;commercial&#x60; 时，响应中会补充更完整的市、区、运营商、时区、海拔等信息，响应时间可能会稍长。
        /// </summary>
        /// <param name="ip">你需要查询的公网IP地址或域名（支持IPv4和IPv6）。</param>
        /// <param name="source">查询结果类型。不传时返回标准结果；如果设置为 &#x60;commercial&#x60;，将返回更完整的地理位置信息，但响应时间可能会稍长。 (optional)</param>
        /// <param name="cancellationToken">Cancellation Token to cancel the request.</param>
        ...
        /// <summary>
        /// 查询我的 IP 想知道你自己的出口公网IP是多少吗？这个接口就是你的“网络身份证”。默认返回标准结果；如果传 &#x60;source&#x3D;commercial&#x60;，可以返回更完整的位置信息。  ## 功能概述 调用此接口，它会返回你（即发起请求的客户端）的公网IP地址，并附带与 &#x60;/network/ipinfo&#x60; 接口相同的地理位置和网络归属信息。非常适合用于在网页上向用户展示他们自己的IP和地理位置。  当传 &#x60;source&#x3D;commercial&#x60; 时，响应中会补充更完整的市、区、运营商、时区、海拔等信息，响应时间可能会稍长。
        /// </summary>
        /// <param name="source">查询结果类型。不传时返回标准结果；如果设置为 &#x60;commercial&#x60;，将返回更完整的地理位置信息，但响应时间可能会稍长。 (optional)</param>
        /// <param name="cancellationToken">Cancellation Token to cancel the request.</param>
        /// <returns><see cref="Task"/>&lt;<see cref="IGetNetworkMyipApiResponse"/>&gt;</returns>
