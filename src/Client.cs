...
public class Client {
    ...
    private readonly bool _disableCacheDefault;
    public ResponseMeta? LastResponseMeta { get; private set; }
    public Client(string baseUrl, string? token = null, bool disableCache = false) {
        var normalized = NormalizeBaseUrl(baseUrl);
        _http = new HttpClient { BaseAddress = new Uri(normalized) };
        _disableCacheDefault = disableCache;
    ...
    private static bool? CoerceBool(object? value) => value switch {
        ...
        JsonElement element when element.ValueKind == JsonValueKind.String && bool.TryParse(element.GetString(), out var parsedBool) => parsedBool,
        JsonElement element when element.ValueKind == JsonValueKind.String && long.TryParse(element.GetString(), out var parsedLong) => parsedLong != 0,
        JsonElement element when element.ValueKind == JsonValueKind.Number && element.TryGetInt64(out var parsedNumber) => parsedNumber != 0,
        _ => null,
    };

    private static bool? ReadDisableCache(Dictionary<string, object?>? args) {
        if (args is null) {
            return null;
        }
        if (args.TryGetValue("disableCache", out var camelValue)) {
            return CoerceBool(camelValue);
        }
        if (args.TryGetValue("disable_cache", out var snakeValue)) {
            return CoerceBool(snakeValue);
        }
        return null;
    }

    private Dictionary<string, object?>? ApplyCacheControl(string method, Dictionary<string, object?>? query, bool? disableCache) {
        if (!string.Equals(method, "GET", StringComparison.OrdinalIgnoreCase)) {
            return query;
        }
        if (query is not null && query.TryGetValue("_t", out var explicitCacheBuster) && explicitCacheBuster is not null) {
            return query;
        }
        var effectiveDisableCache = disableCache ?? _disableCacheDefault;
        if (!effectiveDisableCache) {
            return query;
        }
        var next = query is null ? new Dictionary<string, object?>() : new Dictionary<string, object?>(query);
        next["_t"] = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds();
        return next;
    }
